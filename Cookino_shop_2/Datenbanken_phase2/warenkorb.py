...
def alle_bestellungen(status: str | None = None) -> list[dict[str, Any]]:
    """Liefert alle Bestellungen samt Kunde und Positionen.

    Die Funktion ist für das Admin-/Leiter-Dashboard gedacht. Die zugehörige
    FastAPI-Route muss deshalb vor dem Aufruf die Rolle des Nutzers prüfen.
    Optional kann nach einem Bestellstatus wie ``offen`` oder ``versendet``
    gefiltert werden.
    """
    bedingung = ""
    parameter: list[Any] = []
    if status:
        bedingung = "WHERE b.status = %s"
    ...
    with get_connection() as conn:
        rows = conn.execute(
            ...
            SELECT
                ...
                n.nachname,
                n.email,
                ba.id AS bestellung_artikel_id,
                ba.produkt_variante_id,
                ba.produktname,
                ba.artikelnummer,
                ba.groesse,
                ba.farbe,
            ...
            LEFT JOIN bestellung_artikel ba ON ba.bestellung_id = b.id
            {bedingung}
            ORDER BY b.bestellt_am DESC, ba.produktname
            """,
            parameter,
        ).fetchall()

    bestellungen: dict[str, dict[str, Any]] = {}
    for row in rows:
        schluessel = str(row["bestellung_id"])
        if schluessel not in bestellungen:
            bestellungen[schluessel] = {
                "id": row["bestellung_id"],
                "status": row["status"],
                "gesamtpreis": float(row["gesamtpreis"]),
                "bestellt_am": row["bestellt_am"],
        ...
        if row["bestellung_artikel_id"] is not None:
            bestellungen[schluessel]["positionen"].append(
                {
                    "id": row["bestellung_artikel_id"],
                    "produkt_variante_id": row["produkt_variante_id"],
                    "produktname": row["produktname"],
                    "artikelnummer": row["artikelnummer"],
                    "groesse": row["groesse"],
                    "farbe": row["farbe"],
                    "menge": row["menge"],
                    "einzelpreis": float(row["einzelpreis"]),
                }
            )

    return list(bestellungen.values())
