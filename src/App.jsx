...
import SizePickerModal from './components/SizePickerModal.jsx';
import { ladeKatalog, findeProduktZuBild } from './components/catalog.js';
...
function App() {
  ...
  const [sizePickerProdukt, setSizePickerProdukt] = useState(null);
  const [addMessage, setAddMessage] = useState('');
  const { email, authFetch } = useAuth();
  ...
  useEffect(() => {
    ladeKatalog().then(setKatalog);
  }, []);

  async function aktualisiereWarenkorbZaehler() {
    if (!email) return;
    try {
      const res = await authFetch('/warenkorb');
      const data = await res.json();
      if (res.ok) {
        const gesamtMenge = (data.warenkorb || []).reduce((summe, item) => summe + (item.menge || 0), 0);
        const badge = document.getElementById('cartCount');
        if (badge) badge.textContent = gesamtMenge;
      }
    } catch {
      // Zähler bleibt unverändert, falls Laden fehlschlägt
    }
  }
  ...
  async function fuegeVarianteHinzu(varianteId) {
    try {
      ...
      if (res.ok) {
        setAddMessage('Zum Warenkorb hinzugefügt! 🛍️');
        setTimeout(() => setAddMessage(''), 2500);
        aktualisiereWarenkorbZaehler();
      } else {
        setAddMessage('Hinzufügen fehlgeschlagen.');
        setTimeout(() => setAddMessage(''), 2500);
      }
    } catch {
      setAddMessage('Hinzufügen fehlgeschlagen.');
  ...
  useEffect(() => {
    function handleClick(e) {
      ...
      if (!btn || !btn.textContent.includes('In den Beutel')) return;
      const card = btn.closest('.product-card');
      const img = card?.querySelector('img.product-image');
      const produkt = findeProduktZuBild(katalog, img?.getAttribute('src') || '');
      if (!produkt) return;
      ...
      if (varianten.length === 1) {
        fuegeVarianteHinzu(varianten[0].id);
      } else if (varianten.length > 1) {
        setSizePickerProdukt(produkt);
      }
    }
  ...
  return (
    ...
    <div id="musicControl" className="music-control">
        <input
            type="range"
    ...
    <div id="app-wrapper" className="content-hidden">
        ...
        <header className="playful-header">
            ...
            <div className="header-user-actions">
                ...
                <button className="header-action-btn login-btn magnetic-element neumorphic-btn" id="loginBtn" title="Login" aria-label="Login" onClick={() => setLoginOpen(true)}>👤</button>
                <button className="header-action-btn cart-btn magnetic-element neumorphic-btn" id="cartBtn" title="Warenkorb" aria-label="Warenkorb" onClick={() => setCartOpen(true)}>
                    🛒
        ...
        <div className="infinite-marquee-wrapper" aria-hidden="true">
            <div className="marquee-content">
                ...
                <span>💪🏻 "Dem Suchenden öffne ich das Herz des Waldes und teile das Licht meiner uralten Weisheit. Doch wer die Lehre verschmäht, dem begegne ich als unnachgiebiger Wächter im dichten Schatten der Dornen." 💪🏻</span>
                <span>🍪 "Wahre Größe misst man nicht in Ellen, sondern in der Anzahl der Krümel, die man beim Lachen hinterlässt, und dem Schalk, der in den Augen blitzt." 🍪</span>
                <span>🏠 "Warum sollte ich mein Haus jemals wieder gerade rücken? Es hat vor Jahrhunderten eine Verbeugung vor Freude gemacht – und wer bin ich, diese Höflichkeit rückgängig zu machen?" 🏠</span>
                <span>🍪 "Ein Cookino allein ist nur ein Krümel. Aber wenn wir uns alle an den Händchen halten, werden wir zum Quellcode der Freude und das WLAN unseres Herzens stürzt nie ab." 🍪</span>
                <span>🧙‍♀️ "Wo das Licht auf den vergessenen Zauberspruch trifft, beginnt der Pfad der Erkenntnis; Annora lehrt uns, dass wahre Macht nicht im Zauber selbst liegt, sondern im unermüdlichen Suchen nach dem 'Warum' hinter den Geheimnissen des Waldes." 🧙‍♀️</span>
                <span>📖 "Die Zukunft eines Herzens, das den Mut besitzt, seine eigene Geschichte zu schreiben." 📖</span>
                ...
                <span aria-hidden="true">💪🏻 "Dem Suchenden öffne ich das Herz des Waldes und teile das Licht meiner uralten Weisheit. Doch wer die Lehre verschmäht, dem begegne ich als unnachgiebiger Wächter im dichten Schatten der Dornen." 💪🏻</span>
                <span aria-hidden="true">🍪 "Wahre Größe misst man nicht in Ellen, sondern in der Anzahl der Krümel, die man beim Lachen hinterlässt, und dem Schalk, der in den Augen blitzt." 🍪</span>
                <span aria-hidden="true">🏠 "Warum sollte ich mein Haus jemals wieder gerade rücken? Es hat vor Jahrhunderten eine Verbeugung vor Freude gemacht – und wer bin ich, diese Höflichkeit rückgängig zu machen?" 🏠</span>
                <span aria-hidden="true">🍪 "Ein Cookino allein ist nur ein Krümel. Aber wenn wir uns alle an den Händchen halten, werden wir zum Quellcode der Freude und das WLAN unseres Herzens stürzt nie ab." 🍪</span>
                <span aria-hidden="true">🧙‍♀️ "Wo das Licht auf den vergessenen Zauberspruch trifft, beginnt der Pfad der Erkenntnis; Annora lehrt uns, dass wahre Macht nicht im Zauber selbst liegt, sondern im unermüdlichen Suchen nach dem 'Warum' hinter den Geheimnissen des Waldes." 🧙‍♀️</span>
                <span aria-hidden="true">📖 "Die Zukunft eines Herzens, das den Mut besitzt, seine eigene Geschichte zu schreiben." 📖</span>
            </div>
        </div>
            ...
            <section id="view-stories" className="view-section hidden-view">
                ...
                <div className="monster-grid character-grid">
                    ...
                    <div className="product-card character-card-clickable tilt-card spotlight-card" data-character="moniki">
                        ...
                        <span className="card-role">Die  Tresenhexe</span>
                    ...
                    <div className="product-card character-card-clickable tilt-card spotlight-card" data-character="eternis">
                        ...
                        <img src="/assets/monster_peek.webp" alt="Monster" className="hover-monster" />
                        <img src="/assets/eternis.webp" alt="Codex Eternis" className="product-image parallax-img" />
                        <span className="card-title">Codex Eternis</span>
...
Vor langer, langer Zeit, als der Pixel Plüschwald noch voller Geheimnisse war und die Sterne jede Nacht mit den Bäumen flüsterten, lebten hoch oben über den Wolken unzählige kleine Datenfunken...</p>
    ...
    {loginOpen && (
        <div className="modal-overlay" aria-hidden="false" onClick={(e) => { if (e.target === e.currentTarget) setLoginOpen(false); }}>
            <LoginModal onClose={() => setLoginOpen(false)} />
        </div>
    )}

    {cartOpen && (
        <div className="modal-overlay" aria-hidden="false" onClick={(e) => { if (e.target === e.currentTarget) setCartOpen(false); }}>
            <CartModal onClose={() => { setCartOpen(false); aktualisiereWarenkorbZaehler(); }} />
        </div>
    )}

    {sizePickerProdukt && (
        <div className="modal-overlay" aria-hidden="false" onClick={(e) => { if (e.target === e.currentTarget) setSizePickerProdukt(null); }}>
            <SizePickerModal
                produkt={sizePickerProdukt}
                onClose={() => setSizePickerProdukt(null)}
                onSelect={(varianteId) => {
                    fuegeVarianteHinzu(varianteId);
                    setSizePickerProdukt(null);
                }}
            />
        </div>
    )}
    ...
    {addMessage && (
        <div style={{ position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%)', background: '#231206', color: '#fff', padding: '12px 24px', borderRadius: '50px', zIndex: 9999 }}>
            {addMessage}
        </div>
    )}
